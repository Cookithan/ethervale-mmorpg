...
- ✅ **9. Barre de compétences WoW** (`bfb52b3`) : 4 cases carrées (ATK/sort1/sort2/sort3-set), icônes RPG Ability (`SKILL_ICONS`), bordure dorée + **émeraude** pour le set, raccourci + coût mana + voile/chiffre de cooldown + cadenas (Niv 10 / Set 4/4) + flash. Helper `buildSkillCase`.
- ✨ **BONUS — Mage élémentaire** (`a563ca6`) : l'élément vient de l'apparence (flamme=FEU brûlure, cieux=GLACE ralenti, ombre=OMBRE affaiblit). `MAGE_KITS` + helpers `elementalStorm`/`elementalBolt` + `Monster.applyBurn`/`applyWeaken`. La barre suit l'apparence.
- ✅ **10. Onglet perso (paper-doll MMORPG)** (`2b1e4b9`) : refonte WoW — thème **cuir/bois + or** (dégradé, coins ornés), **faceset** + lueur de classe, 4 slots en croix **grands** (bordure de qualité + **halo pulsant** épique/légendaire/set + survol + silhouettes Ninja UI CC0), section **Set récompense** (« Set de <classe> x/4 » + bonus 2/4 + compétence touche 3, `SETS.skillDesc`), **stats à icônes** (⚔️🛡️❤️💧). **Accès** : bouton HUD « Perso » (bas-gauche) + portrait HUD cliquable + C. **Inventaire visible** pendant la fiche (voile troué autour des cases du sac, cliquables → équiper rafraîchit en direct).

## ✅ BRIEF A TERMINÉ (2026-06-05) — les 10 étapes faites + bonus Mage élémentaire. Assets : Kyrise (items), Admurin (armes), RPG Ability Icons (barre), Ninja UI (fiche perso), FX Ninja (Ice/Rock/Spark/Magic). Reste possible : équilibrage, armes de set spectaculaires (ligne émeraude Admurin déjà repérée), polish.
