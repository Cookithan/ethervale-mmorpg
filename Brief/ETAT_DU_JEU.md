...
> ⚡ **INTÉRIEURS (2026-06-07, poussé sur `main`)** : ✅ **TAVERNE refaite et validée** (bar pleine largeur au fond, infranchissable, barman + bouteilles + tabourets, tables à chaises tournées, sol bois foncé, murs avec ombre de profondeur) — « on garde, à peaufiner plus tard ». Invite **« Entrer ? » Oui/Non recentrée**. ⏳ **Apothicaire** = ancien jet (à refaire), **Auberge/Banque** non faites. **👉 Méthode qui marche = `scripts/room_preview.cjs`** (compositeur hors-jeu qui rend une salle en PNG → fini le « placement à l'aveugle ») + catalogue `Brief/penzilla-catalog.md`. Détail + reprise : `Brief/brief-interieurs-village.md` (bloc « ⚠️ HANDOFF » en tête).
>
